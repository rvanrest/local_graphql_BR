...
// Builds a WHERE clause for Bemiddelingspecificatie that understands
// the nested `bemiddeling` relation filter by emitting an EXISTS subquery.
function buildBsFilter(filter) {
  if (!filter) return { conditions: "", params: [] };
  const parts = [];
  const params = [];
  ...
  function processFilter(f) {
    for (const [field, opInput] of Object.entries(f)) {
      if (field === "and" && Array.isArray(opInput)) {
        const subs = opInput.map((sub) => buildBsFilter(sub));
        const valid = subs.filter((s) => s.conditions);
        if (valid.length) {
          parts.push("(" + valid.map((s) => s.conditions).join(" AND ") + ")");
          valid.forEach((s) => params.push(...s.params));
      ...
      // Relation: bemiddeling → emit EXISTS subquery
      if (
        field === "bemiddeling" &&
        typeof opInput === "object" &&
        opInput !== null
      ) {
        const rel = buildFilter(opInput, bemiddelingFields);
        if (rel.conditions) {
          parts.push(
            `EXISTS (SELECT 1 FROM Bemiddeling b ` +
              `WHERE b.bemiddelingID = Bemiddelingspecificatie.bemiddelingID ` +
              `AND (${rel.conditions}))`,
          );
      ...
      // Regular scalar field on Bemiddelingspecificatie
      if (!bemiddelingspecificatieFields[field]) continue;
      if (typeof opInput !== "object" || opInput === null) continue;
      const col = bemiddelingspecificatieFields[field];
      for (const [op, val] of Object.entries(opInput)) {
        if (val !== undefined) applyOp(col, op, val, parts, params);
      }
    }
...
function toWhereClauseBs(filter) {
  const { conditions, params } = buildBsFilter(filter);
  return { clause: conditions ? `WHERE ${conditions}` : "", params };
}
...
const bemiddelingspecificatieFields = {
  ...
  pgbPercentage: "pgbPercentage",
  opname: "opname",
  redenIntrekking: "redenIntrekking",
  etmalen: "etmalen",
  instellingBestemming: "instellingBestemming",
...
const overdrachtFields = {
  ...
  verantwoordelijkZorgkantoor: "verantwoordelijkZorgkantoor",
  overdrachtDatum: "overdrachtDatum",
...
const contactpersoonFields = {
  ...
  voorvoegselPartnernaam: "voorvoegselPartnernaam",
  voornamen: "voornamen",
  ...
  geboortedatumgebruik: "geboortedatumgebruik",
  ingangsdatum: "ingangsdatum",
  einddatum: "einddatum",
};
...
const clientContactgegevensFields = {
  ...
  land: "land",
  aanduidingWoonadres: "aanduidingWoonadres",
  emailadres: "emailadres",
  telefoonnummer01: "telefoonnummer01",
  landnummer01: "landnummer01",
  telefoonnummer02: "telefoonnummer02",
  landnummer02: "landnummer02",
  ingangsdatum: "ingangsdatum",
  einddatum: "einddatum",
};
...
function buildResolvers(db) {
  return {
    Query: {
      ...
      //      },
      bemiddelingspecificatie: (_, { where }) => {
        const { clause, params } = toWhereClauseBs(where);
        return queryAll(
          db,
          `SELECT * FROM Bemiddelingspecificatie ${clause}`,
          params,
        );
      },
      overdracht: (_, { where }) => {
    ...
    Client: {
      ...
      bemiddeling: (p, { where, order }) => {
        const { clause, params } = mergeWhere(
          "clientID",
          p.clientID,
          where,
          bemiddelingFields,
        ...
        return queryAll(
          db,
          `SELECT * FROM Bemiddeling ${clause} ${buildOrder(order, bemiddelingFields)}`,
          params,
        );
      },
      contactpersoon: (p, { where, order }) => {
        ...
        return queryAll(
          db,
          `SELECT * FROM Contactpersoon ${clause} ${buildOrder(order, contactpersoonFields)}`,
          params,
        );
      },
      contactgegevens: (p, { where, order }) => {
        const { clause, params } = mergeWhere(
          ...
          p.clientID,
          where,
          clientContactgegevensFields,
        );
        return queryAll(
          db,
          `SELECT * FROM ClientContactgegevens ${clause} ${buildOrder(order, clientContactgegevensFields)}`,
          params,
        );
      },
    },
    ...
    Bemiddeling: {
      ...
      bemiddelingspecificatie: (p, { where, order }) => {
        const { clause, params } = mergeWhere(
          ...
          p.bemiddelingID,
          where,
          bemiddelingspecificatieFields,
        );
        return queryAll(
          db,
          `SELECT * FROM Bemiddelingspecificatie ${clause} ${buildOrder(order, bemiddelingspecificatieFields)}`,
          params,
        );
      },
      regiehouder: (p, { where, order }) => {
        ...
        return queryAll(
          db,
          `SELECT * FROM Regiehouder ${clause} ${buildOrder(order, regiehouderFields)}`,
          params,
        );
      },
      overdracht: (p) =>
        queryOne(db, "SELECT * FROM Overdracht WHERE bemiddelingID = ?", [
          p.bemiddelingID,
        ]),
    },
    ...
    Bemiddelingspecificatie: {
      ...
      pgbPercentage: (p) =>
        p.pgbPercentage != null ? Number(p.pgbPercentage) : null,
      bemiddeling: (p) =>
        queryOne(db, "SELECT * FROM Bemiddeling WHERE bemiddelingID = ?", [
          p.bemiddelingID,
        ]),
      overdrachtspecificatie: (p) =>
        queryOne(
          db,
          "SELECT * FROM Overdrachtspecificatie WHERE bemiddelingspecificatieID = ?",
          [p.bemiddelingspecificatieID],
        ),
    },
    ...
    Overdracht: {
      ...
      overdrachtspecificatie: (p, { where, order }) => {
        const { clause, params } = mergeWhere(
          ...
          p.overdrachtID,
          where,
          overdrachtspecificatieFields,
        );
        return queryAll(
          db,
          `SELECT * FROM Overdrachtspecificatie ${clause} ${buildOrder(order, overdrachtspecificatieFields)}`,
          params,
        );
      },
    },
    ...
    Overdrachtspecificatie: {
      ...
      overdracht: (p) =>
        queryOne(db, "SELECT * FROM Overdracht WHERE overdrachtID = ?", [
          p.overdrachtID,
        ]),
      bemiddelingspecificatie: (p) =>
        queryOne(
          db,
          "SELECT * FROM Bemiddelingspecificatie WHERE bemiddelingspecificatieID = ?",
          [p.bemiddelingspecificatieID],
        ),
    },
    ...
    Contactpersoon: {
      ...
      contactgegevens: (p, { where, order }) => {
        const { clause, params } = mergeWhere(
          ...
          p.contactpersoonID,
          where,
          contactpersoonContactgegevensFields,
        );
        return queryAll(
          db,
          `SELECT * FROM ContactpersoonContactgegevens ${clause} ${buildOrder(order, contactpersoonContactgegevensFields)}`,
          params,
        );
      },
    },
    ...
    ContactpersoonContactgegevens: {
      ...
      contactpersoon: (p) =>
        queryOne(
          db,
          "SELECT * FROM Contactpersoon WHERE contactpersoonID = ?",
          [p.contactpersoonID],
        ),
    },
  };
...
async function main() {
  ...
  createServer(yoga).listen(4000, () => {
    console.log("");
    console.log("  ✅  iWlz Bemiddelingsregister GraphQL server running (version 2024.06)");
